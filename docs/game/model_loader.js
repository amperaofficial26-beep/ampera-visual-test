...
export async function loadVillageModels(scene) {
  ...
  // Skala diturunkan agar rumah tetap lebih besar dari player tanpa menutup kamera/jalan.
  housesLeft.forEach((h, i) => jobs.push(place(scene, i % 2 ? 'houseB' : 'houseA', { position:[h[0],h[1],h[2]], rotationY:Math.PI / 2 + h[3], scale:h[4] * 1.45, name:`model_house_left_${i}` })));
  housesRight.forEach((h, i) => jobs.push(place(scene, i % 2 ? 'houseA' : 'houseB', { position:[h[0],h[1],h[2]], rotationY:-Math.PI / 2 + h[3], scale:h[4] * 1.45, name:`model_house_right_${i}` })));
